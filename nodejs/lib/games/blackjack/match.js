...
module.exports = Blackjack_Match;
...
function Blackjack_Match (set, match = false) {
  this._super.call(this, 'blackjack', set, match);
}
Blackjack_Match.prototype = Object.create(Match.prototype);
Blackjack_Match.prototype.constructor = Blackjack_Match;
Blackjack_Match.prototype._super = Match;

// Public Methods

Blackjack_Match.prototype.start = function () {
...
Blackjack_Match.prototype.started = function () {
  this.engine = Engine.new_game(this.player_names(), {
    hit_16: this.settings.hit_16,
    stand_17: this.settings.stand_17,
...
Blackjack_Match.prototype.make_move = function (player, move) {
...
Blackjack_Match.prototype.valid_move = function (move) {
...
Blackjack_Match.prototype.resume = function () {
...
Blackjack_Match.prototype.play_ai = function (play_now) {
  ...
  Engine.ai_move(match.engine, match.state.to_play, function (rtrn) {
    match.engine = rtrn;
...
Blackjack_Match.prototype.get_alive = function (one = false) {
  if (this.has_started()) {
...
Blackjack_Match.prototype.finish = function (turn, decision) {
  var i;
  ...
  fileout == '[Hit 16 ' + this.settings.hit_16+ ']\n';
  fileout == '[Stand 17 ' + this.settings.stand_17 + ']\n';
  fileout += '[Result %out]\n\n';
...
Blackjack_Match.prototype.to_save = function () {
...
function update_state (match, update = true) {
  ...
  for (i = 0; i < nbr_players; i++) {
    ...
    match.state.setting.players[i] = {
      bet: engine.players[i].round_bet,
      cards: [],
      chips: engine.players[i].bankroll,
      status: engine.players[i].status
    };

    for (var j = 0; j < match.hands[i].cards.length; j++) {
      if (
        j > 0 ||
        engine.show_cards === true ||
        engine.button_i !== i ||
        engine.button_i === engine.to_play
      ) {
        match.state.setting.players[i].cards.push(match.hands[i].cards[j]);
      }
      else {
        match.state.setting.players[i].cards.push(null);
      }
    }
  }
